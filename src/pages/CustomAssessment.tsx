...
Темы вопросов должны покрывать основные аспекты темы "${topic}" и позволять определить уровень знаний от начинающего до эксперта.`;
      ...
      const response = await fetch('https://api.openai.com/v1/chat/completions', {
        ...
        body: JSON.stringify({
          ...
          messages: [
            ...
            {
              role: 'user',
              content: `Создай 5 вопросов для тестирования уровня знаний по теме "${topic}". Верни только JSON массив вопросов.`
            }
          ],
          max_tokens: 2000,
          temperature: 0.7,
      ...
      const generatedQuestions = JSON.parse(content.replace(/```json\n?|\n?```/g, ''));
